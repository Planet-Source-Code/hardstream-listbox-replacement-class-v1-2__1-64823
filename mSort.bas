...
'|Verson: 2.1                                                               |
'/==========================================================================\
'|HardStream likes to thank Rde for his work. This module is entirely       |
'|written by Rde.                                                           |
'|The code has been reformatted by HardStream, following the HardStream     |
...
'Private constants (type/enumeration-bound)
Private Const Default_Direction = Ascending
...
If mDirection = Equal Then mDirection = Default_Direction
...
Sub strSwapStabletemp(sA() As String, ByVal lbA As Long, ByVal ubA As Long, Optional ByVal CaseSensitive As VbCompareMethod, Optional ByVal Direction As eDirection = Default) '-©Rd 2006-
If Not InitedArray(sA, lbA, ubA) Then Exit Sub
If Direction = Default Then Direction = SortDirection
...
Sub strSwapStableIdxtemp(sA() As String, lIdxA() As Long, ByVal lbA As Long, ByVal ubA As Long, Optional ByVal CaseSensitive As VbCompareMethod, Optional ByVal Direction As eDirection = Default) '-©Rd 2006-
If Not InitedArray(sA, lbA, ubA) Then Exit Sub
If Direction = Default Then Direction = SortDirection
SortCompare = Direction
CaseSensitivity = CaseSensitive
...
'ascending order, or capitals first in descending order).
'
...
Do: ptr1 = n0: ptr2 = n0
    ...
    If lPrettyReverse <> n0 Then
        ...
        If (ptr2 = n0) Then lPrettyReverse = 50000
    End If
...
Sub strPrettySort(sA() As String, ByVal lbA As Long, ByVal ubA As Long, Optional ByVal CapsFirst As Boolean = True, Optional ByVal Direction As eDirection = Default)
...
If bReDim = False Then bReDim = lbA < LB Or ubA > UB
If bReDim Then
...
If (iDataType And vbArray) = vbArray Then 'if a valid string array was passed
    ...
    If InitedArray Then
        ...
        If bTestRedimable Then InitedArray = InitedArray + ((tSA.fFeatures And FADF_FIXEDSIZE) <> FADF_FIXEDSIZE)
...
Function strVerifySort(sA() As String, ByVal lbA As Long, ByVal ubA As Long, Optional ByVal CaseSensitive As VbCompareMethod, Optional ByVal Direction As eDirection = Default) As Boolean
...
Function strVerifyIndexed(sA() As String, lA() As Long, ByVal lbA As Long, ByVal ubA As Long, Optional ByVal CaseSensitive As VbCompareMethod, Optional ByVal Direction As eDirection = Default) As Boolean
On Error GoTo FreakOut
Dim walk As Long
